...
// ---> sikk - trigger_secret functionality
	if ( activator && activator->IsType( idPlayer::Type ) ) {
		idPlayer* player = static_cast< idPlayer * >( activator );
		if ( spawnArgs.GetBool( "secret" ) ) {
			const char *sound;
			player->spawnArgs.GetString( "snd_secret", "", &sound );
			player->StartSoundShader( declManager->FindSound( sound ), SND_CHANNEL_ANY, 0, false, NULL );
			player->AddToLevelStat( SECRETSFOUND, 1 );
		}

		player->sticky = 1.0f - spawnArgs.GetFloat( "stickyness" );	// sikk - trigger_sticky
	}
// <--- sikk - trigger_secret functionality
