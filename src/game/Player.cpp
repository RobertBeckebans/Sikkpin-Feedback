...
const int WEAPON_DROP_TIME = 20000;

// time before a next or prev weapon switch happens
const int WEAPON_SWITCH_DELAY = 0;	//150;	// sikk - No weapon switch delay
...
void idInventory::Clear( void ) {
	...
	//statsScoreTotal = 0;	// sikk - Level Stats System
	//rechargerAmount = 0;	// sikk - Recharger Pack System
}
...
void idInventory::GivePowerUp( idPlayer *player, int powerup, int msec ) {
	if ( !msec ) {
		...
		switch ( powerup ) {
			case MEGADAMAGE:
				def = gameLocal.FindEntityDef( "powerup_megadamage", false );
				break;
			case MEGASHIELD:
				def = gameLocal.FindEntityDef( "powerup_megashield", false );
				break;
			case MEGAENERGY:
				def = gameLocal.FindEntityDef( "powerup_megaenergy", false );
				break;
			case MEGACHARGE: 
				def = gameLocal.FindEntityDef( "powerup_megacharge", false );
				break;
		}
		assert( def );
		msec = SEC2MS( def->dict.GetInt( "time" ) );
	}
	powerups |= 1 << powerup;
	powerupEndTime[ powerup ] = gameLocal.time + msec * player->modifiers.misc_poweruptimedoubler;	// sikk - Modifier System
	powerupEndSoundPlayed[ powerup ] = false;		// sikk - Powerup End Sounds
}
...
void idInventory::ClearPowerUps( void ) {
	...
	for ( i = 0; i < MAX_POWERUPS; i++ ) {
		powerupEndTime[ i ] = 0;
		powerupEndSoundPlayed[ 1 ] = false;		// sikk - Powerup End Sounds
	}
	powerups = 0;
}
...
void idInventory::RestoreInventory( idPlayer *owner, const idDict &dict ) {
	...
	maxEnergy = dict.GetInt( "maxEnergy", "100" );
...
// ---> sikk - Powerup End Sounds
	for ( i = 0; i < MAX_POWERUPS; i++ ) {
		savefile->WriteBool( powerupEndSoundPlayed[ i ] );
	}
// <--- sikk - Powerup End Sounds

	//savefile->WriteInt( statsScoreTotal );	// sikk - Level Stats System
	//savefile->WriteInt( rechargerAmount );	// sikk - Recharger Pack System
}
...
// ---> sikk - Powerup End Sounds
	for ( i = 0; i < MAX_POWERUPS; i++ ) {
		savefile->ReadBool( powerupEndSoundPlayed[ i ] );
	}
// <--- sikk - Powerup End Sounds

	//savefile->ReadInt( statsScoreTotal );	// sikk - Level Stats System
	//savefile->ReadInt( rechargerAmount );	// sikk - Recharger Pack System
}
...
bool idInventory::Give( idPlayer *owner, const idDict &spawnArgs, const char *statname, const char *value, int *idealWeapon, bool updateHud ) {
	...
	if ( !idStr::Icmp( statname, "megadamage" ) ) {
		GivePowerUp( owner, MEGADAMAGE, SEC2MS( atof( value ) ) );
	} else if ( !idStr::Icmp( statname, "megaenergy" ) ) {
		GivePowerUp( owner, MEGAENERGY, SEC2MS( atof( value ) ) );
...
// ---> sikk - New Initializations
	...
	underwater				= false;		// sikk - Liquid Physics
	sticky					= 1.0f;			// sikk - trigger_sticky
	velocityDamageScale		= 1.0f;			// sikk - Forward Velocity Damage Multiplier

	lastEnergyTallyTime		= 0;
	lastFireTime			= 0;			// sikk - Energy Take
	nextHealthGive			= 0;			// sikk - Energy Give

	focusMoveableTimer		= 0;			// sikk - Object Manipulation

	focusDistance			= 0.0f;			// sikk - Depth of Field PostProcess

	nextScoreThreshold		= 100000;		// sikk - Lives System
...
// ---> sikk - Combo Multiplier
	comboFull				= false;
	comboTime				= 0;
	comboMultiplier			= 0.0f;
// <--- sikk -

// ---> sikk - Level Stats System
	ClearLevelStats();
	SetLevelStat( LIVES, 3 - g_skill.GetInteger() );
// <--- sikk - Level Stats System
...
// sikk - removed multiplayer
	//if ( !gameLocal.isMultiplayer ) {
		...
		//if ( health < spawnArgs.GetInt( "basehealth" ) ) {
			health = spawnArgs.GetInt( "basehealth" );
			energy = spawnArgs.GetInt( "baseenergy" );
		//}
		g_damageScale.SetFloat( 1.0f );

		ClearModifier();						// sikk - Modifier System

		healthTake = true;
		nextHealthTake = gameLocal.time + SEC2MS( g_healthTakeTime.GetFloat() );
	//}
}
...
/*	if ( spectating ) {
	...
	UpdateRespawnPos();	// sikk - Lives System

	UpdateForwardVelocity();	// sikk - Forward Velocity Damage Multiplier
	...
	UpdateModifiers();// sikk - Modifier System

	UpdateDeathSkin( false );

	UpdateMusic();	// sikk - Level Music

	UpdateLevelStats();	// sikk - Level Stats System
...
void idPlayer::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteInt( healthPool );
	savefile->WriteInt( energyPool );
	...
	savefile->WriteInt( nextHealthGive );	// sikk - Energy Give
...
// sikk - removed multiplayer
	...
	savefile->WriteBool( underwater );			// sikk - Liquid Physics
	savefile->WriteInt( energy );				// sikk - Energy
	savefile->WriteInt( energyTally );			// sikk - Energy Tally
	savefile->WriteInt( lastEnergyTallyTime );	// sikk - Energy Tally
	savefile->WriteInt( nextScoreThreshold );	// sikk - Lives System

	SaveLevelStats( savefile );	// sikk - Level Stats System
...
// sikk - removed multiplayer
	...
	savefile->ReadBool( underwater );			// sikk - Liquid Physics: used for underwater post fx
	savefile->ReadInt( energy );				// sikk - Energy
	savefile->ReadInt( energyTally );			// sikk - Energy Tally
	savefile->ReadInt( lastEnergyTallyTime );	// sikk - Energy Tally
	savefile->ReadInt( nextScoreThreshold );	// sikk - Lives System
// ---> sikk - Level Stats System
...
void idPlayer::SavePersistantInfo( void ) {
	...
	playerInfo.SetInt( "energy", energy );
	playerInfo.SetInt( "current_weapon", currentWeapon );

	playerInfo.SetInt( "statsScoreTotal", GetTotalScore() ); // sikk - Level Stats System
	playerInfo.SetInt( "nextScoreThreshold", nextScoreThreshold );	// sikk - Lives System
}
...
// sikk - removed multiplayer
	...
	energy = spawnArgs.GetInt( "baseenergy", "100" );	// sikk - Start Fresh with each level
// sikk - removed multiplayer
	//if ( !gameLocal.isClient ) {
		idealWeapon = spawnArgs.GetInt( "current_weapon", "1" );
	//}

	SetLevelStat( SCORETOTAL, spawnArgs.GetInt( "statsScoreTotal", "0" ) );	// sikk - Level Stats System
	nextScoreThreshold = spawnArgs.GetInt( "nextScoreThreshold", "100000" );	// sikk - Lives System
}
...
void idPlayer::UpdateHudStats( idUserInterface *_hud ) {
	...
	_hud->SetStateInt( "showtime", g_showHudTime.GetInteger() );
	_hud->SetStateInt( "showspeed", g_showHudSpeed.GetInteger() );
	_hud->SetStateInt( "showscore", g_showHudScore.GetInteger() );

	_hud->SetStateInt( "player_energysystem", g_energySystem.GetInteger() );

	_hud->SetStateInt( "player_health", health );
	_hud->SetStateInt( "player_energy", energy );

	if ( g_energySystem.GetBool() ) {
		if ( health >= inventory.maxHealth && energy < inventory.maxEnergy ) {
			_hud->SetStateInt( "player_healthtotal", health );
			_hud->SetStateInt( "player_energytotal", idMath::ClampInt( 0, 800, energy + healthPool ) );
		} else if ( health < inventory.maxHealth && energy >= inventory.maxEnergy ) {
			_hud->SetStateInt( "player_healthtotal", idMath::ClampInt( 0, 200, health + healthPool ) );
			_hud->SetStateInt( "player_energytotal", energy );
		} else {
			_hud->SetStateInt( "player_healthtotal", idMath::ClampInt( 0, 200, health + healthPool * 0.2f ) );
			_hud->SetStateInt( "player_energytotal", idMath::ClampInt( 0, 800, energy + healthPool * 0.8f ) );
		}
	} else {
		_hud->SetStateInt( "player_healthtotal", idMath::ClampInt( 0, 1000, health + healthPool ) );
	}
	_hud->HandleNamedEvent( "updateArmorHealthAir" );

	if ( healthPulse ) {
		_hud->HandleNamedEvent( "healthPulse" );	// sikk - Hud Positive Energy Pulse
		StartSound( "snd_healthpulse", SND_CHANNEL_ITEM, 0, false, NULL );
		healthPulse = false;
	}

	if ( healthTake ) {
		_hud->HandleNamedEvent( "healthTake" );	// sikk - Hud Negative Energy Pulse
		StartSound( "snd_healthtake", SND_CHANNEL_ITEM, 0, false, NULL );
		healthTake = false;
	}

//	gameLocal.globalShaderParms[ 6 ] = comboMultiplier;
//	renderView->shaderParms[ 6 ] = comboMultiplier;
	_hud->SetStateFloat( "player_combomultiplier", comboMultiplier );
	_hud->SetStateInt( "player_combotime", SEC2MS( g_comboTime.GetFloat() * modifiers.misc_combotimedoubler ) );	// sikk - Modifier System

	_hud->SetStateFloat( "player_damagescale", velocityDamageScale );

	_hud->SetStateInt( "player_speed", GetPhysics()->GetLinearVelocity().Length() );

	_hud->SetStateInt( "player_score", GetLevelStat( SCORE ) );

	float dist = 0.0f;
	if ( gameLocal.endLevelRadius[ 0 ] > 0.0f ) {
		dist = idVec3( gameLocal.endLevelPos[ 0 ] - GetPhysics()->GetOrigin() ).Length() / gameLocal.endLevelRadius[ 0 ];
		for ( int i = 1; i < 4; i++ ) {
			if ( gameLocal.endLevelRadius[ i ] > 0.0f ) {
				float f = idVec3( gameLocal.endLevelPos[ i ] - GetPhysics()->GetOrigin() ).Length() / gameLocal.endLevelRadius[ i ];
				dist = dist > f ? f : dist;
			}
		}
	}
	if ( dist ) {
		dist = 1.0f - idMath::ClampFloat( 0.0f, 1.0f, dist );
	}
	_hud->SetStateFloat( "player_exitmsg", dist );

// ---> sikk - Lives System
	_hud->SetStateInt( "player_respawnmsg", ( GetLevelStat( LIVES ) && health <= 0 && gameLocal.time > minRespawnTime ) );
	_hud->SetStateInt( "player_lives", GetLevelStat( LIVES ) );

	int n = nextScoreThreshold < 100000 ? 100000 : nextScoreThreshold;
	float respawneramount = ( GetLevelStat( SCORE ) - ( n - 100000 ) ) * 0.00001f;
	_hud->SetStateFloat( "player_respawneramount", respawneramount );
// <--- sikk - Lives System

	int time = MS2SEC( GetLevelStat( TIME ) );
	int timeMin = time / 60;
	int timeSec = time % 60;
	if ( timeSec < 10 ) {
		_hud->SetStateString( "player_time", va( "%d:0%d", timeMin, timeSec ) );
	} else {
		_hud->SetStateString( "player_time", va( "%d:%d", timeMin, timeSec ) );
	}

// ---> sikk - Boss health meter
	idActor *ent;
	int bossTotalHealth = 0;
	int bossCurrentHealth = 0;
	for ( ent = enemyList.Next(); ent != NULL; ent = ent->enemyNode.Next() ) { 
		if ( ent->GetFinalBoss() ) {
			bossTotalHealth += ent->spawnArgs.GetInt( "health" );
			bossCurrentHealth += ent->health;
		}
	}

	_hud->SetStateInt( "boss_health", bossCurrentHealth );
	_hud->SetStateFloat( "boss_healthtotal", 1.0f / bossTotalHealth );
// <--- sikk - Boss health meter

// ---> sikk - Keys
	_hud->SetStateInt( "player_key_red", FindInventoryItem( "Red Key" ) ? 1 : 0 );
	_hud->SetStateInt( "player_key_green", FindInventoryItem( "Green Key" ) ? 1 : 0 );
	_hud->SetStateInt( "player_key_blue", FindInventoryItem( "Blue Key" ) ? 1 : 0 );
// <--- sikk - Keys

	_hud->SetStateInt( "player_modifier", activeModifier );	// sikk - Modifier System
}
...
// sikk - draw combo timer here
	float alpha = (float)( comboTime - gameLocal.time ) / SEC2MS( g_comboTime.GetFloat() * modifiers.misc_combotimedoubler );	// sikk - Modifier System
	alpha = ( alpha < 0.0f ) ? 0.0f : alpha;
	renderSystem->SetColor4( 0.0f, 0.5f, 0.75f, alpha );
	renderSystem->DrawStretchPic( 4, 444, 32, 32, 0.0f, 0.0f, 1.0f, 1.0f, declManager->FindMaterial( "gui/hud/combomultiplierFill" ) );
...
==================
*/
void idPlayer::WeaponFireFeedback( const idDict *weaponDef, int primary ) {
	// force a blink
	blink_time = 0;
	...
	playerView.WeaponFireFeedback( weaponDef, primary );
}
...
===============
*/
void idPlayer::FireWeapon( bool primary ) {
	idMat3 axis;
	idVec3 muzzle;
	...
	if ( !hiddenWeapon && weapon.GetEntity()->IsReady() ) {
		if ( GetEnergy() > weapon.GetEntity()->spawnArgs.GetInt( "minEnergy", "20" ) ) {
			AI_ATTACK_HELD = !primary;
			weapon.GetEntity()->BeginAttack( primary );// sikk - Secondary Attack
		} else if ( !( ( usercmd.buttons & BUTTON_ATTACK ) && ( oldButtons & BUTTON_ATTACK ) ) &&
					!( ( usercmd.buttons & BUTTON_5 ) && ( oldButtons & BUTTON_5 ) ) ) {
			NextBestWeapon();
		}
	}
}
...
bool idPlayer::Give( const char *statname, const char *value ) {
	...
	if ( !idStr::Icmp( statname, "health" ) ) {
		int amount = atoi( value ) * modifiers.energy_pickupdoubler;	// sikk - Modifier System
		if ( amount ) {
			if ( g_energySystem.GetBool() ) {
				health += amount;
				energy += amount;
				health = health > inventory.maxHealth ? inventory.maxHealth : health;
				energy = energy > inventory.maxEnergy ? inventory.maxEnergy : energy;
			} else {
				int maxenergy = inventory.maxHealth + inventory.maxEnergy;
				health += amount;
				health = health > maxenergy ? maxenergy : health;
			}

			nextHealthTake = gameLocal.time + SEC2MS( g_healthTakeDelay.GetFloat() );

			GiveEnergyTally( amount );
			if ( hud ) {
				hud->HandleNamedEvent( "itemPickup" );
				hud->SetStateString( "energyTally", va( "+%d Energy", GetEnergyTally() ) );
				hud->HandleNamedEvent( "energyTallyFlash" );
			}
		}
	...
	} else if ( !idStr::Icmp( statname, "modifier" ) ) {
		GiveModifier( atoi( value ) );	// sikk - Modifier System
	} else if ( !idStr::Icmp( statname, "score" ) ) {
		AddToLevelStat( SCORE, atoi( value ) * modifiers.misc_scoredoubler );	// sikk - Level Stats System && Modifier System
	} else {
...
===============
*/
void idPlayer::GiveHealthPool( int amt ) {
	if ( AI_DEAD ) {
		return;
	}
	...
	if ( health > 0 ) {
		...
		if ( g_energySystem.GetBool() ) {
			if ( healthPool > maxenergy - ( health + energy ) ) {
				healthPool = maxenergy - ( health + energy );
			}
		} else {
			if ( healthPool > maxenergy - health ) {
				healthPool = maxenergy - health;
			}
		}
...
===============
*/
bool idPlayer::GivePowerUp( int powerup, int time, int score ) {	// sikk - Level Stats System: Added score
	const char *sound;
	const char *skin;
...
// sikk - removed multiplayer
		...
		if ( powerup != MEGACHARGE ) {	// sikk - Lives System
		...
		switch( powerup ) {
			case MEGADAMAGE: {
				spawnArgs.GetString( "skin_megadamage", "", &skin );
				...
				if ( spawnArgs.GetString( "snd_megadamage", "", &sound ) ) {
			...
			case MEGASHIELD: {
				spawnArgs.GetString( "skin_megashield", "", &skin );
				powerUpSkin = declManager->FindSkin( skin );
				// remove any decals from the model
				if ( modelDefHandle != -1 ) {
					gameRenderWorld->RemoveDecals( modelDefHandle );
				...
				if ( spawnArgs.GetString( "snd_megashield", "", &sound ) ) {
					StartSoundShader( declManager->FindSound( sound ), SND_CHANNEL_ANY, 0, false, NULL );
				}
				break;
			}
// ---> sikk - Lives System
			case MEGACHARGE: {
				AddToLevelStat( LIVES, 1 );
				hud->HandleNamedEvent( "livesPulse" );
// <--- sikk - Lives System
				break;
			 }
			case MEGAENERGY: {
				if ( spawnArgs.GetString( "snd_megaenergy", "", &sound ) ) {
					StartSoundShader( declManager->FindSound( sound ), SND_CHANNEL_ANY, 0, false, NULL );
				}
				def = gameLocal.FindEntityDef( "powerup_megaenergy", false );
				if ( def ) {
		...
		AddToLevelStat( SCORE, score * modifiers.misc_scoredoubler );	// sikk - Level Stats System && Modifier System
...
// sikk - removed multiplayer
	...
	inventory.powerupEndSoundPlayed[ i ] = false;	// sikk - Powerup End Sounds
	if ( i == MEGADAMAGE ) {
...
// sikk - removed multiplayer
	//if ( !gameLocal.isClient ) {
		for ( int i = 0; i < MAX_POWERUPS; i++ ) {
			if ( PowerUpActive( i ) ) {
// ---> sikk - Powerup End Sounds
				if ( inventory.powerupEndTime[ i ] <= gameLocal.time + 3000 && !inventory.powerupEndSoundPlayed[ i ] ) {
					if ( i == MEGADAMAGE ) {
						StartSound( "snd_megadamage_end", SND_CHANNEL_ANY, SSF_PRIVATE_SOUND, false, NULL );
					} else if ( i == MEGASHIELD ) {
						StartSound( "snd_megashield_end", SND_CHANNEL_ANY, SSF_PRIVATE_SOUND, false, NULL );
					} else if ( i == MEGAENERGY ) {
						StartSound( "snd_megaenergy_end", SND_CHANNEL_ANY, SSF_PRIVATE_SOUND, false, NULL );
					}
					inventory.powerupEndSoundPlayed[ i ] = true;
				}
// <--- sikk - Powerup End Sounds
				if ( inventory.powerupEndTime[ i ] <= gameLocal.time ) {
					ClearPowerup( i );
				}
			}
		}
	...
	if ( g_energySystem.GetBool() && health > inventory.maxHealth ) {
		health = inventory.maxHealth;
	}

// ---> sikk - Energy Give/Take
	if ( !gameLocal.inCinematic && !influenceActive && !AI_DEAD && health > 0 ) {
		if ( healthPool && 
			 gameLocal.time > nextHealthPulse && 
			 gameLocal.time > ( lastDmgTime + SEC2MS( g_healthpoolGiveDelay.GetFloat() ) * modifiers.energy_rechargerate ) ) {

			assert( !gameLocal.isClient );	// healthPool never be set on client
			int maxenergy = inventory.maxHealth + inventory.maxEnergy;
			int amt = ( healthPool > g_healthpoolGiveAmt.GetInteger() ) ? g_healthpoolGiveAmt.GetInteger() : healthPool;
			...
			nextHealthPulse = gameLocal.time + SEC2MS( g_healthpoolGiveTime.GetFloat() ) * modifiers.energy_rechargerate;	// sikk - Modifier System
			nextHealthTake = gameLocal.time + SEC2MS( g_healthTakeDelay.GetFloat() ) * modifiers.energy_conservation;	// sikk - delay health take while we're gaining energy
			nextHealthGive = gameLocal.time + SEC2MS( g_healthGiveTime.GetFloat() ) * modifiers.energy_rechargerate;	// sikk - Modifier System

			healthPulse = true;
		}

		float modifier = g_energySystem.GetBool() ? 0.5f : 1.0f;
		if ( !PowerUpActive( MEGAENERGY ) && 
			 ( health > g_healthTakeLimit.GetInteger() * modifier ) && 
			 gameLocal.time > nextHealthTake && 
			 gameLocal.time > ( lastDmgTime + SEC2MS( g_healthTakeDelay.GetFloat() ) * modifiers.energy_conservation ) && 
			 gameLocal.time > ( lastFireTime + SEC2MS( g_healthTakeDelay.GetFloat() ) * modifiers.energy_conservation ) ) {

			if ( !( g_skill.GetInteger() == 0 && g_energySystem.GetBool() ) ) {
				health -= g_healthTakeAmt.GetInteger();
				if ( health < g_healthTakeLimit.GetFloat() * modifier ) {
					health = g_healthTakeLimit.GetFloat() * modifier;
				}
				nextHealthTake = gameLocal.time + SEC2MS( g_healthTakeTime.GetFloat() ) * modifiers.energy_conservation;
				healthTake = true;
			}
		}

		if ( g_skill.GetInteger() == 0 && 
			 health < g_healthGiveLimit.GetInteger() && 
			 gameLocal.time > nextHealthGive && 
			 gameLocal.time > ( lastDmgTime + SEC2MS( g_healthGiveDelay.GetFloat() ) ) ) {

			health += g_healthGiveAmt.GetInteger();
			if ( health > g_healthGiveLimit.GetInteger() ) {
				health = g_healthGiveLimit.GetInteger();
			}
			nextHealthGive = gameLocal.time + SEC2MS( g_healthGiveTime.GetFloat() );
			healthPulse = true;
		}
	}
// <--- sikk - Energy Give/Take
...
// ---> sikk - Lives System
	if ( health >= 0 && GetLevelStat( SCORE ) >= nextScoreThreshold ) {
		AddToLevelStat( LIVES, 1 );	
		nextScoreThreshold += 100000;
		hud->HandleNamedEvent( "livesPulse" );
	}
// <--- sikk - Lives System
}
...
//	info.icon = item->GetString( "inv_icon" );
	...
	if ( hud && !info.icon.IsEmpty() ) {
		hud->SetStateString( "player_centerprintmsg", info.icon );
		hud->HandleNamedEvent( "centerprintMsg" );

		gameLocal.Printf( "**********************************\n" );
		gameLocal.Printf( info.icon );
		gameLocal.Printf( "\n**********************************\n" );
	}
	return true;
}
...
// sikk - removed multiplayer
	...
	while ( w > 0 ) {
		...
		if ( !spawnArgs.GetBool( va( "weapon%d_best", w ) ) || GetEnergy() <= spawnArgs.GetInt( va( "weapon%d_minenergy", w ) ) ) {
			continue;
		}
		break;
	}
...
// sikk - removed multiplayer
	...
	while ( 1 ) {
		...
		if ( GetEnergy() > spawnArgs.GetInt( va( "weapon%d_minenergy", w ) ) ) {
			break;
		}
	}
...
//		num = weapon_fists;
	...
	if ( force || ( inventory.weapons & ( 1 << num ) ) ) {
		if ( GetEnergy() <= spawnArgs.GetInt( va( "weapon%d_minenergy", num ) ) && !spawnArgs.GetBool( va( "weapon%d_allowempty", num ) ) ) {
			return;
		}
		if ( ( previousWeapon >= 0 ) && ( idealWeapon == num ) && ( spawnArgs.GetBool( va( "weapon%d_toggle", num ) ) ) ) {
			...
			if ( GetEnergy() <= spawnArgs.GetInt( va( "weapon%d_minenergy", num ) ) && !spawnArgs.GetBool( va( "weapon%d_allowempty", previousWeapon ) ) ) {
				return;
			}
...
// sikk - removed multiplayer
		if ( ( usercmd.buttons & BUTTON_ATTACK ) /*&& !weaponGone*/ ) {
			FireWeapon( true );
		} else if ( ( usercmd.buttons & BUTTON_5 ) /*&& !weaponGone*/ ) {
			FireWeapon( false );
		} else if ( ( oldButtons & BUTTON_ATTACK ) || ( oldButtons & BUTTON_5 ) ) {
			AI_ATTACK_HELD = false;
			weapon.GetEntity()->EndAttack();
			if ( GetEnergy() <= weapon.GetEntity()->spawnArgs.GetInt( "minEnergy", "20" ) ) {
				NextBestWeapon();
...
// ---> sikk - Crosshair Color
	crosshairColor = colorWhite;
	start = firstPersonViewOrigin;
	end = start + firstPersonViewAxis[ 0 ] * 32768.0f;
	gameLocal.clip.TracePoint( trace, start, end, MASK_SHOT_RENDERMODEL, this );
	if ( trace.c.entityNum ) {
		idEntity* ent = gameLocal.entities[ trace.c.entityNum ];
		if ( ent && ent->IsType( idActor::Type ) ) {
			float health = ent->spawnArgs.GetInt( "health" );
			float pct =  (float)ent->health / health;
			if ( pct < 0.33333334f ) {
				crosshairColor = colorRed;
			} else if ( pct < 0.66666667f ) {
				crosshairColor = colorYellow;
			} else {
				crosshairColor = colorGreen;
			}
		}
	}
	crosshairColor.w = g_crosshairAlpha.GetFloat();
// <--- sikk - Crosshair Color
...
// ---> sikk - Liquid Physics	
	...
	idPhysics_Player *phys = dynamic_cast<idPhysics_Player *>( GetPhysics() );
...
// <--- sikk - Liquid Physics
	...
	if ( newAirless ) {
		...
		if ( airTics < 0 ) {
			...
			int dmgTiming = SEC2MS( damageDef ? damageDef->GetFloat( "delay", "3.0" ) : 3.0f );
...
// ---> sikk - Level Stats System: GUI
	if ( scoreBoardOpen ) {
		stats->Activate( true, gameLocal.time );
	} else {
		stats->Activate( false, gameLocal.time );
	}
// <--- sikk - Level Stats System: GUI
}
...
// sikk - removed multiplayer
	...
	switch( impulse ) {
		...
		case IMPULSE_18: {
			ToggleScoreboard();	// sikk - PDA removed, now it's a simple scoreboard toggle
			break;
		}
...
// sikk - removed multiplayer
		if ( /*forceRespawn && !gameLocal.isMultiplayer &&*/ !g_testDeath.GetBool() && restart ) {	// sikk - Lives System
...
//sikk	scoreBoardOpen = ( ( usercmd.buttons & BUTTON_SCORES ) != 0 || forceScoreBoard );
...
/*	if ( spectating ) {
		...
		bobFrac = 0.0f;
	} else if ( usercmd.buttons & BUTTON_RUN ){
	...
	speed *= modifiers.player_speed;									// sikk - Modifier System
	crouchspeed = pm_crouchspeed.GetFloat() * modifiers.player_speed;	// sikk - Modifier System
	ladderspeed = pm_ladderspeed.GetFloat() * modifiers.player_speed;	// sikk - Modifier System
...
// ---> sikk - stickyness
	physicsObj.SetSpeed( speed * sticky, crouchspeed * sticky, ladderspeed * sticky );
	sticky += 0.01666667f;
	if ( sticky > 1.0f )
		sticky = 1.0f;
// <--- sikk - stickyness
...
// ---> sikk - Modifier System
	physicsObj.SetAirAcceleration( modifiers.player_aircontrol );
	physicsObj.SetDoubleJump( modifiers.player_doublejump );
// <--- sikk - Modifier System
...
// ---> sikk - Crosshair Cvar
	if ( g_crosshair.GetBool() ) {
		float aspect = (float)renderSystem->GetScreenHeight() / (float)renderSystem->GetScreenWidth() + 0.25f;
		cursor->SetStateString( "combatcursor", "1" );
		cursor->SetStateInt( "crosshairstyle", g_crosshair.GetInteger() );
		cursor->SetStateString( "crosshaircolor", crosshairColor.ToString() );
		cursor->SetStateFloat( "crosshairscaleX", g_crosshairScale.GetFloat() * aspect );
		cursor->SetStateFloat( "crosshairscaleY", g_crosshairScale.GetFloat() );
	} else {
...
// sikk - removed multiplayer
	...
	if ( health <= 0 ) {
		if ( !doingDeathSkin ) {
			...
			if ( state_hitch ) {
				renderEntity.shaderParms[ SHADERPARM_TIME_OF_DEATH ] = MS2SEC( gameLocal.time ) - 2.0f;
			} else {
...
void idPlayer::DamageFeedback( idEntity *victim, idEntity *inflictor, int &damage ) {
	assert( !gameLocal.isClient );
//	damage *= PowerUpActive( MEGADAMAGE ) * 3.0f + 1.0f; // sikk - Why is this called here? 
...
// sikk - removed multiplayer
	...
	if ( PowerUpActive( MEGASHIELD ) ) {
		StartSound( "snd_hitShield", SND_CHANNEL_ANY, SSF_PRIVATE_SOUND, false, NULL );
		// still do knockback when invulnerable
...
//		int oldHealth = health;	// sikk - variable initialized but not used
		health -= damage;
		AddToLevelStat( DAMAGETAKEN, damage );	// sikk - Level Stats System 
		hud->HandleNamedEvent( "damageFlash" );	// sikk - Hud Damgage Energy Pulse
...
// sikk - edit this function to remove weapon bobbing
	// gun angles from bobbing
	angles.roll		= scale * bobfracsin * 0.001f;
	angles.yaw		= scale * bobfracsin * 0.001f;
	...
	angles.roll		+= 0.0f;	//fracsin;
	angles.yaw		+= 0.0f;	//fracsin;
	angles.pitch	+= 0.0f;	//fracsin;
...
=============
*/
void idPlayer::AddAIKill( int _energy ) {
// ---> sikk - Level Stats System
	// increment kill counter
	AddToLevelStat( KILLS, 1 );
	AddToLevelStat( SCORE, _energy * modifiers.misc_scoredoubler );	// sikk - Modifier System
// <--- sikk - Level Stats System

// ---> sikk - Give energy * combo multiplier for kill
	float multiplier = ( comboMultiplier >= 1.0f ) ? g_comboMultiplier.GetFloat() : 1.0f;
	GiveHealthPool( _energy * g_energyPct.GetFloat() * multiplier );
	
	musicTime = gameLocal.time + SEC2MS( g_musicTime.GetFloat() );
	comboTime = gameLocal.time + SEC2MS( g_comboTime.GetFloat() * modifiers.misc_combotimedoubler );	// sikk - Modifier System
	comboMultiplier += 0.25f;

	if ( comboMultiplier >= 1.0f ) {
		comboMultiplier = 1.0f;
		hud->HandleNamedEvent( "comboMultiplier" );
		if ( !comboFull ) {
			StartSound( "snd_combo_full", SND_CHANNEL_ANY, 0, false, NULL );
			comboFull = true;
		} else {
			AddToLevelStat( COMBOS, 1 );
		}
	}
// <--- sikk - Give energy
}
...
void idPlayer::AddProjectilesFired( int count ) {
	AddToLevelStat( SHOTSFIRED, count );	// sikk - Level Stats System
}
...
void idPlayer::AddProjectileHits( int count ) {
	AddToLevelStat( SHOTSHIT, count );	// sikk - Level Stats System
	// sikk - hit feedback sound moved here from SetLastHitTime()
	if ( PowerUpActive( MEGADAMAGE ) ) {
		StartSound( "snd_hit_feedback_megadamage", SND_CHANNEL_ANY, SSF_PRIVATE_SOUND, false, NULL );
	} else {
...
void idPlayer::SpawnMusic( void ) {
	...
	ent = gameLocal.FindEntity( "music_critical" );
	if ( ent == NULL ) {
		args.Set( "classname", "speaker" );
		args.Set( "name", "music_critical" );
		args.Set( "s_shader", "music_critical" );
		args.SetVector( "origin", GetPhysics()->GetOrigin() );
		gameLocal.SpawnEntityDef( args, &ent );
		gameLocal.program.ReturnEntity( ent );
		args.Clear();
	}
	ent = gameLocal.FindEntity( "music_low" );
	if ( ent == NULL ) {
		const char* music_low;
		gameLocal.world->spawnArgs.GetString( "music_low", "", &music_low );
		args.Set( "classname", "speaker" );
		args.Set( "name", "music_low" );
		args.Set( "s_shader", music_low );
		args.SetVector( "origin", GetPhysics()->GetOrigin() );
		gameLocal.SpawnEntityDef( args, &ent );
		gameLocal.program.ReturnEntity( ent );
		args.Clear();
	}
	ent = gameLocal.FindEntity( "music_high" );
	if ( ent == NULL ) {
		const char* music_high;
		gameLocal.world->spawnArgs.GetString( "music_high", "", &music_high );
		args.Set( "classname", "speaker" );
		args.Set( "name", "music_high" );
		args.Set( "s_shader", music_high );
		gameLocal.SpawnEntityDef( args, &ent );
		gameLocal.program.ReturnEntity( ent );
		args.Clear();
	}
...
void idPlayer::UpdateMusic( void ) {
	...
	float fadetimeCritical, fadetimeLow, fadetimeHigh;
	float volumeMain = s_volumeMusic.GetFloat();
	int oldFlag = musicFlag;
	...
	if ( doEndLevel ) {
		musicFlag = -1;
		volumeCritical		= -40.0f;
		fadetimeCritical	= 0.0f;
		volumeLow			= -40.0f;
		fadetimeLow			= 0.0f;
		volumeHigh			= -40.0f;
		fadetimeHigh		= 0.0f;
		...
		volumeCritical		= -40.0f;
		fadetimeCritical	= 0.0f;
		volumeLow			= volumeMain;
		fadetimeLow			= 2.0f;
		volumeHigh			= -40.0f;
		fadetimeHigh		= 0.0f;
	} else if ( health < 25 ) {
		musicFlag			= 2;
		volumeCritical		= volumeMain;
		fadetimeCritical	= 2.0f;
		volumeLow			= -40.0f;
		fadetimeLow			= 2.0f;
		volumeHigh			= -40.0f;
		fadetimeHigh		= 2.0f;
		...
		volumeCritical		= -40.0f;
		fadetimeCritical	= 1.0f;

		if ( gameLocal.time > musicTime ) {
			musicFlag		= 3;
			volumeLow		= volumeMain;
			fadetimeLow		= 2.0f;
			volumeHigh		= -40.0f;
			...
			if ( comboFull ) {
				...
				volumeLow		= -40.0f;
				fadetimeLow		= 1.0f;
				volumeHigh		= volumeMain + 10.0f;
				fadetimeHigh	= 0.5f;
			} else {
				musicFlag		= 5;
				volumeLow		= -40.0f;
				fadetimeLow		= 5.0f;
				volumeHigh		= volumeMain ;
				fadetimeHigh	= 2.0f;
			}
		}
	}

	if ( oldFlag != musicFlag ) {
		ent = gameLocal.FindEntity( "music_critical" );
		if ( ent ){
			ent->FadeSound( SND_CHANNEL_ANY, volumeCritical, fadetimeCritical );
		}
		ent = gameLocal.FindEntity( "music_low" );
		if ( ent ){
			ent->FadeSound( SND_CHANNEL_ANY, volumeLow, fadetimeLow );
		}
		ent = gameLocal.FindEntity( "music_high" );
		if ( ent ){
			ent->FadeSound( SND_CHANNEL_ANY, volumeHigh, fadetimeHigh );
		}
	}
}
// <--- sikk - Level Music
...
int idPlayer::GetTotalScore( void ) {
	...
	if ( GetLevelStat( SHOTSFIRED ) ) {
		acc = ( (float)GetLevelStat( SHOTSHIT ) / GetLevelStat( SHOTSFIRED ) ) * 100;
	} else {
		acc = 0.0f;
	}
	total = GetLevelStat( SCORETOTAL ) + GetLevelStat( SCORE ) + GetLevelStat( ENERGY ) + GetLevelStat( COMBOS ) + GetLevelStat( KILLS ) + 
			( GetLevelStat( LIVES ) * 100 ) + ( GetLevelStat( SECRETSFOUND ) * 100 )  +
			( ( GetLevelStat( DAMAGEDEALT ) - GetLevelStat( DAMAGETAKEN ) ) * 0.01 ) + ( acc * 10 );
...
/*
===============
idPlayer::GetLevelStat
==============
*/
int idPlayer::GetLevelStat( int stat ) {
	int i = -1;
	switch ( stat ) {
	case TIME: 
		i = levelStats.time; break;
	case SCORE:
		i = levelStats.score; break;
	case SCORETOTAL:
		i = levelStats.scoreTotal; break;
	case LIVES:
		i = levelStats.lives; break;
	case KILLS:
		i = levelStats.kills; break;
	case ENEMYCOUNT:
		i = levelStats.enemyCount; break;
	case SECRETSFOUND:
		i = levelStats.secretsFound; break;
	case SECRETSTOTAL:
		i = levelStats.secretsTotal; break;
	case ENERGY:
		i = levelStats.energy; break;
	case COMBOS:
		i = levelStats.combos; break;
	case SHOTSFIRED:
		i = levelStats.shotsFired; break;
	case SHOTSHIT:
		i = levelStats.shotsHit; break;
	case DAMAGEDEALT:
		i = levelStats.damageDealt; break;
	case DAMAGETAKEN:
		i = levelStats.damageTaken; break;
	}
...
/*
===============
idPlayer::SetLevelStat
==============
*/
void idPlayer::SetLevelStat( int stat, int amount ) {
	switch ( stat ) {
	case TIME: 
		levelStats.time			= amount; break;
	case SCORE:
		levelStats.score		= amount; break;
	case SCORETOTAL:
		levelStats.scoreTotal	= amount; break;
	case LIVES:
		levelStats.lives		= amount; break;
	case KILLS:
		levelStats.kills		= amount; break;
	case ENEMYCOUNT:
		levelStats.enemyCount	= amount; break;
	case SECRETSFOUND:
		levelStats.secretsFound	= amount; break;
	case SECRETSTOTAL:
		levelStats.secretsTotal	= amount; break;
	case ENERGY:
		levelStats.energy		= amount; break;
	case COMBOS:
		levelStats.combos		= amount; break;
	case SHOTSFIRED:
		levelStats.shotsFired	= amount; break;
	case SHOTSHIT:
		levelStats.shotsHit		= amount; break;
	case DAMAGEDEALT:
		levelStats.damageDealt	= amount; break;
	case DAMAGETAKEN:
		levelStats.damageTaken	= amount; break;
	}
}

/*
===============
idPlayer::AddToLevelStat
==============
*/
void idPlayer::AddToLevelStat( int stat, int amount ) {
	switch ( stat ) {
	case TIME:
		levelStats.time			+= amount; break;
	case SCORE:
...
/*
===============
idPlayer::SaveLevelStats
==============
*/
void idPlayer::SaveLevelStats( idSaveGame *savefile ) const {
	savefile->WriteInt( levelStats.time	);
	savefile->WriteInt( levelStats.score );
	savefile->WriteInt( levelStats.scoreTotal );
	savefile->WriteInt( levelStats.lives );
	savefile->WriteInt( levelStats.kills );
	savefile->WriteInt( levelStats.enemyCount );
	savefile->WriteInt( levelStats.secretsFound );
	savefile->WriteInt( levelStats.secretsTotal );
	savefile->WriteInt( levelStats.energy );
	savefile->WriteInt( levelStats.combos );
	savefile->WriteInt( levelStats.shotsFired );
	savefile->WriteInt( levelStats.shotsHit );
	savefile->WriteInt( levelStats.damageDealt );
	savefile->WriteInt( levelStats.damageTaken );
}
...
void idPlayer::UpdateLevelStats( void ) {
	...
	// only update time when in play
	if ( !doEndLevel ) {
		AddToLevelStat( TIME, 16 );
	}
	time = MS2SEC( GetLevelStat( TIME ) );
	timeMin = time / 60;
	timeSec = time % 60;
	SetLevelStat( ENERGY, g_energySystem.GetBool() ? ( health + energy ) : health );

	SetLevelStat( ENEMYCOUNT, gameLocal.enemyCount );
	SetLevelStat( SECRETSTOTAL, gameLocal.secretsCount );

	if ( health > 0 ) {
		if ( timeSec < 10 ) {
			stats->SetStateString( "player_statsTime", va( "%d:0%d", timeMin, timeSec ) );
		} else {
			stats->SetStateString( "player_statsTime", va( "%d:%d", timeMin, timeSec ) );
		}
		stats->SetStateInt( "player_statsScore", GetLevelStat( SCORE ) );
		stats->SetStateInt( "player_statsLives", GetLevelStat( LIVES ) );
		stats->SetStateString( "player_statsSecrets", va( "%d/%d", GetLevelStat( SECRETSFOUND ), GetLevelStat( SECRETSTOTAL ) ) );
		stats->SetStateInt( "player_statsEnergy", GetLevelStat( ENERGY ) );
		stats->SetStateInt( "player_statsCombos", GetLevelStat( COMBOS ) );
		stats->SetStateString( "player_statsKills", va( "%d/%d", GetLevelStat( KILLS ), GetLevelStat( ENEMYCOUNT ) ) );
		stats->SetStateInt( "player_statsDamageDealt", GetLevelStat( DAMAGEDEALT ) );
		stats->SetStateInt( "player_statsDamageTaken", GetLevelStat( DAMAGETAKEN ) );
		stats->SetStateInt( "player_statsShotsFired", GetLevelStat( SHOTSFIRED ) );
		stats->SetStateInt( "player_statsShotsHit", GetLevelStat( SHOTSHIT ) );

		if ( levelStats.shotsFired ) {
			acc = ( (float)levelStats.shotsHit / levelStats.shotsFired ) * 100;
			stats->SetStateString( "player_statsAccuracy", va( "%.1f%%", acc ) );
		} else {
			acc = 0.0f;
			stats->SetStateString( "player_statsAccuracy", "0.0%" );
		}

		stats->SetStateInt( "player_statsScoreTotal", GetTotalScore() );
	}
}
// <--- sikk - Level Stats System
...
==================
*/
void idPlayer::Respawn() {
	if ( levelStats.lives && health <= 0 && !influenceActive ) {
		Teleport( respawnPos, respawnAng, NULL );
		health = spawnArgs.GetInt( "basehealth" );
		energy = spawnArgs.GetInt( "baseenergy" );
		GivePowerUp( MEGASHIELD, SEC2MS( 3.0f ) );
		fl.noknockback = false;
		levelStats.lives--;
	}
}
...
void idPlayer::UpdateRespawnPos( void ) {
	if ( health > 0 && !AI_DEAD && AI_ONGROUND && !AI_CROUCH && !AI_ONLADDER ) {
		respawnPos = GetPhysics()->GetOrigin();
		respawnAng = viewAngles;
		respawnAng.pitch = 0.0f;
	}
}
// <--- sikk - Lives System
...
/*
==================
idPlayer::UpdateForwardVelocity
==================
*/
void idPlayer::UpdateForwardVelocity() {
	idVec3 playerMoveDir = physicsObj.GetLinearVelocity();
	idVec3 playerViewDir = firstPersonViewAxis[ 0 ];
	float playerVelocity = playerMoveDir.Normalize();
	playerVelocity *= idMath::ClampFloat( 0.0f, 1.0f, playerMoveDir * playerViewDir );
	velocityDamageScale = idMath::ClampFloat( 0.0f, 2.0f, playerVelocity / ( pm_runspeed.GetFloat() * 2.0f ) );
}
// <--- sikk - Forward Velocity Damage Multiplier

// ---> sikk - Energy Management
/*
==================
idPlayer::GiveEnergy
==================
*/
void idPlayer::GiveEnergy( int amt ) {
	if ( g_energySystem.GetBool() ) {
		energy += amt;
		if ( energy > inventory.maxEnergy ) {
			energy = inventory.maxEnergy;
...
/*
==================
idPlayer::GiveEnergyTally
==================
*/
void idPlayer::GiveEnergyTally( int amt ) {
	if ( gameLocal.time < lastEnergyTallyTime + 2000 ) {
		energyTally += amt;
	} else {
		energyTally = amt;
	}
	lastEnergyTallyTime = gameLocal.time;
}
// <--- sikk - Energy Management
...
void idPlayer::ResetModifiers( void ) {
	...
	modifiers.player_aircontrol			= 2.0f;

	modifiers.weapon_projectilespeed	= 1;
	modifiers.weapon_projectiledoubler	= 1;
	modifiers.weapon_incendiary			= false;

	modifiers.energy_rechargerate		= 1.0f;
	modifiers.energy_conservation		= 1.0f;
	modifiers.energy_pickupdoubler		= 1;

	modifiers.misc_scoredoubler			= 1;
	modifiers.misc_poweruptimedoubler	= 1;
	modifiers.misc_combotimedoubler		= 1;
}
...
void idPlayer::UpdateModifiers( void ) {
	...
	switch ( GetActiveModifier() ) {
		case MOD_PLAYER_SPEED:				modifiers.player_speed				= 1.5f;		break;
		case MOD_PLAYER_DOUBLEJUMP:			modifiers.player_doublejump			= true;		break;
		case MOD_PLAYER_AIRCONTROL:			modifiers.player_aircontrol			= 10.0f;	break;

		case MOD_WEAPON_PROJECTILESPEED:	modifiers.weapon_projectilespeed	= 2;		break;
		case MOD_WEAPON_PROJECTILEDOUBLER:	modifiers.weapon_projectiledoubler	= 2;		break;
		case MOD_WEAPON_INCENDIARY:			modifiers.weapon_incendiary			= true;		break;

		case MOD_ENERGY_RECHARGERATE:		modifiers.energy_rechargerate		= 0.5f;		break;
		case MOD_ENERGY_CONSERVATION:		modifiers.energy_conservation		= 2.0f;		break;
		case MOD_ENERGY_PICKUPDOUBLER:		modifiers.energy_pickupdoubler		= 2;		break;

		case MOD_MISC_SCOREDOUBLER:			modifiers.misc_scoredoubler			= 2;		break;
		case MOD_MISC_POWERUPTIMEDOUBLER:	modifiers.misc_poweruptimedoubler	= 2;		break;
		case MOD_MISC_COMBOTIMEDOUBLER:		modifiers.misc_combotimedoubler		= 2;		break;
	}
}
// <--- sikk - Modifier System
