...
//		return 0;	// sikk - warning C4702: unreachable code
	}
...
idVarDef *idCompiler::EmitFunctionParms( int op, idVarDef *func, int startarg, int startsize, idVarDef *object ) {
	...
	if ( returnType->Type() == ev_string ) {
		...
		default :
			...
			// shut up compiler
//			resultOp = OP_STORE_OBJ;	// sikk - warning C4702: unreachable code
			break;
		}
	}
...
idVarDef *idCompiler::GetTerm( void ) {
	...
	if ( !immediateType && CheckToken( "~" ) ) {
		...
		default :
			...
			// shut up compiler
//			op = OP_COMP_F;	// sikk - warning C4702: unreachable code
			break;
		}
	...
	if ( !immediateType && CheckToken( "!" ) ) {
		...
		case ev_function :
			...
			// shut up compiler
//			op = OP_NOT_F;	// sikk - warning C4702: unreachable code
			break;
	...
	if ( !immediateType && CheckToken( "-" ) ) {
		...
		if ( immediateType == &type_float ) {
			...
			default :
				...
				// shut up compiler
//				op = OP_NEG_F;	// sikk - warning C4702: unreachable code
				break;
			}
