...
// ---> sikk - Liquid Physics	
	...
	if ( SetWaterLevelf() != 0.0f ) {	
		idVec3 bCenter;
		idVec3 bForce( gravityVector ), rForce( -gravityVector );
		float bMass, fraction, liquidMass;
		bool inWater;

		inWater = GetBuoyancy( next.i.position, next.i.orientation.Transpose(), bCenter, fraction );

		// calculate water mass
		liquidMass = volume * water->GetDensity() * fraction;
		// don't let liquid mass get too high
		liquidMass = Min( liquidMass, 3 * mass );

		bMass = mass - liquidMass;
...
// <--- sikk - Liquid Physics
	...
	// if there was a collision	// ---> sikk - Liquid Physics - Added "collided || "
	...
	if ( gameLocal.clip.Motion( waterCollision, current.i.position, pos, rotation, clipModel, current.i.orientation, MASK_WATER, self ) ) {
		...
		if ( !collided || waterCollision.fraction < collision.fraction ) {
			...
			if ( ent->GetPhysics()->IsType( idPhysics_Liquid::Type ) ) {
				...
				SetWater( liquid );
				water->Splash( self, volume, info, waterCollision );
			}
		}
	}
// <--- sikk - Liquid Physics
...
// ---> sikk - Liquid Physics	
	...
	if ( water != NULL ) {
		if ( current.i.linearMomentum.LengthSqr() < WATER_STOP_LINEAR.LengthSqr() &&
			 current.i.angularMomentum.LengthSqr() < WATER_STOP_ANGULAR.LengthSqr() ) {
		
			if( noMoveTime == 0 ) {
				noMoveTime = gameLocal.GetTime();	
			} else if ( noMoveTime + NO_MOVE_TIME < gameLocal.GetTime() ) {
				noMoveTime = 0;
				return true;
			}
		} else {
...
void idPhysics_RigidBody::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteFloat( volume );	// sikk - Liquid Physics
}
...
void idPhysics_RigidBody::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadFloat( volume );	// sikk - Liquid Physics
}
...
void idPhysics_RigidBody::SetClipModel( idClipModel *model, const float density, int id, bool freeOld ) {
	...
	volume = mass / density;	// sikk - Liquid Physics
...
bool idPhysics_RigidBody::GetBuoyancy( const idVec3 &pos, const idMat3 &rotation, idVec3 &bCenter, float &percent ) const {
	...
	idVec3 tbCenter( pos );
	idBounds bounds = GetBounds();
	idTraceModel tm = *GetClipModel()->GetTraceModel();
	int i, count;
	...
	if ( percent == 1.0f ) {
		...
		for ( i = 0, count = 1; i < tm.numVerts; i++ ) {
			if ( gameLocal.clip.Contents( tm.verts[ i ], NULL, GetAxis(), MASK_WATER, NULL ) ) {
				tbCenter += tm.verts[ i ];
				count += 1;
			}
		}
