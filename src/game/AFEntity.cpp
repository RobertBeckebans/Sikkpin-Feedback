...
// ---> sikk - Rail Charge
	railChargeParticleModelDefHandle = -1;
	railChargeParticleLightDefHandle = -1;
	railChargeParticleTime = 0;
	railChargeTime = 0;
	railCharge = 0;
// <--- sikk - Rail Charge
}
...
// ---> sikk - Rail Charge
	if ( railChargeParticleModelDefHandle != -1 ) {
		gameRenderWorld->FreeEntityDef( railChargeParticleModelDefHandle );
		railChargeParticleModelDefHandle = -1;
	}
	if ( railChargeParticleLightDefHandle != -1 ) {
		gameRenderWorld->FreeLightDef( railChargeParticleLightDefHandle );
		railChargeParticleLightDefHandle = -1;
	}
// <--- sikk - Rail Charge
}
...
void idAFEntity_Gibbable::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, const char *damageDefName, const float damageScale, const int location ) {
	...
	if ( health < spawnArgs.GetInt( "gibHealth", "-20" ) && spawnArgs.GetBool( "gib" ) ) {	// sikk - Always gib
...
// <--- sikk - Random Item drops
	...
	for ( i = 0; i < list.Num(); i++ ) {
		if ( gibNonSolid ) {
			...
			list[i]->GetPhysics()->SetLinearVelocity( velocity * 200.0f );	// sikk - was 75.0f
		}
...
// ---> sikk - Rail Charge
	if ( railCharge ) {
		// create particle fx
		idEntityFx::StartFx( "fx/railcharge_explosion.fx", &GetPhysics()->GetAbsBounds().GetCenter(), &GetPhysics()->GetAxis(), NULL, false );
	}
// <--- sikk - Rail Charge
