...
// ---> sikk - Level Stats System: Add damage to level stat
		if( attacker && attacker->IsType( idPlayer::Type ) && inflictor )
		{
			static_cast< idPlayer* >( attacker )->AddToLevelStat( DAMAGEDEALT, damage );
		}
// <--- sikk - Level Stats System
		...
		if( health <= 0 )
		{
			...
			if( attacker->IsType( idPlayer::Type ) && static_cast<idPlayer*>( attacker )->comboFull )
			{
				forceGib = true;
			}

			Killed( inflictor, attacker, damage, dir, location );
			if( ( health < spawnArgs.GetInt( "gibHealth", "-20" ) && spawnArgs.GetBool( "gib" ) && damageDef->GetBool( "gib" ) ) || forceGib )  	// sikk - Always gib
			{
