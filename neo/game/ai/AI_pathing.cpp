...
const float MAX_OBSTACLE_RADIUS			= 64.0f;//256.0f;
const float PUSH_OUTSIDE_OBSTACLES		= 0.5f;
const float CLIP_BOUNDS_EPSILON			= 10.0f;
const int 	MAX_AAS_WALL_EDGES			= 16.0f;//256;
const int 	MAX_OBSTACLES				= 16.0f;//256;
const int	MAX_PATH_NODES				= 16.0f;//256;
const int 	MAX_OBSTACLE_PATH			= 16;//64;
...
// sikk - This is causing a crash. Why?
	if( !pathToGoalExists )
	{
...
bool idAI::PredictPath( const idEntity* ent, const idAAS* aas, const idVec3& start, const idVec3& velocity, int totalTime, int frameTime, int stopEvent, predictedPath_t& path )
{
	...
	for( i = 0; i < numFrames; i++ )
	{
		...
		delta = curVelocity * MS2SEC( curFrameTime );
		...
		// add gravity
		curVelocity += gravity * MS2SEC( frameTime );
	}
...
=====================
*/
// sikk - warning C4505: 'HeightForTrajectory' : unreferenced local function has been removed
