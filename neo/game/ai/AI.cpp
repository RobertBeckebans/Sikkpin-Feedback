...
void idAI::Think( void )
{
	...
	*/
	...
	UpdateIncendiaryBurn();	// sikk - Modifier system

// ---> sikk - Rail Charge
	if( gameLocal.time > railChargeTime + 100 && railCharge < 100 )
	{
		railChargeTime = gameLocal.time;
		railCharge--;
		if( railCharge < 0 )
		{
			railCharge = 0;
	...
	if( railCharge == 100 )
	{
		...
		renderEntity.shaderParms[ 1 ] = 0.0f;
		renderEntity.shaderParms[ 2 ] = 0.0f;
		renderEntity.shaderParms[ 3 ] = 1.0f;
	}
	else
	{
		float f = ( float )railCharge * 0.01f;
		renderEntity.shaderParms[ 0 ] = 0.0f;
		renderEntity.shaderParms[ 1 ] = f * 0.66666667f;
		renderEntity.shaderParms[ 2 ] = f;
		renderEntity.shaderParms[ 3 ] = f;
	}
// <--- sikk - Rail Charge
}
...
void idAI::AnimMove( void )
{
	...
	{
// sikk		if ( move.moveCommand != MOVE_WANDER ) {
...
void idAI::SlideMove( void )
{
	...
	{
// sikk		CheckObstacleAvoidance( goalPos, newDest );
//		TurnToward( newDest );
//		goalPos = newDest;
		TurnToward( goalPos );	// sikk - Added
	}
...
// ---> sikk - Modifier System
	if( spawnArgs.GetBool( "canDropModifier", "1" ) && ( gameLocal.random.RandomFloat() * 0.99999f ) < g_modifierChance.GetFloat() )
	{
		idDict args;
		idEntity* ent;
		args.Set( "classname", GetRandomModifier() );
		args.Set( "origin", physicsObj.GetOrigin().ToString() );
		gameLocal.SpawnEntityDef( args, &ent );
		ent->GetPhysics()->SetContents( CONTENTS_CORPSE );
		ent->GetPhysics()->SetClipMask( CONTENTS_SOLID );
		ent->GetPhysics()->SetLinearVelocity( idVec3( 0, 0, 400 ) );
	}
// <--- sikk - Modifier System
...
// ---> sikk - Rail Charge
	if( railCharge > 0 )
	{
		StartSound( "rocket_impact", SND_CHANNEL_BODY, 0, true, NULL );
		gameLocal.RadiusDamage( physicsObj.GetOrigin(), gameLocal.GetLocalPlayer(), gameLocal.GetLocalPlayer(), gameLocal.GetLocalPlayer(), this, "damage_plasmaCharge", ( float )railCharge * 0.01f );
	}
// <--- sikk - Rail Charge

// ---> sikk - Leech energy from kill for both the player and ai
	if( attacker && attacker->IsType( idPlayer::Type ) && inflictor )
	{
		static_cast< idPlayer* >( attacker )->AddAIKill( spawnArgs.GetInt( "health" ) );
	}
	else if( attacker && attacker->IsType( idAI::Type ) && inflictor )
	{
		static_cast< idAI* >( attacker )->health += spawnArgs.GetInt( "health" );
	}
// <--- sikk - Leech energy from kill for both the player and ai
}
