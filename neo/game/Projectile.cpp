...
// ---> sikk - Modifier System
			if( player->modifiers.weapon_incendiary )
			{
				const idDict* damageDef = gameLocal.FindEntityDefDict( damageDefName );
				if( damageDef )
				{
					ent->burnPower += damageDef->GetInt( "damage" ) * damageScale * 0.1f;
					if( ent->burnPower < 1 )
					{
						ent->burnPower = 1;
					}
...
// <--- sikk - Modifier System
			...
			if( ent->IsType( idActor::Type ) )
			{
				...
				damageScale *= player->PowerUpActive( MEGADAMAGE ) ? 4.0f : 1.0f;
...
//			}
//		} else  {
//			if ( accelTime && gameLocal.time < launchTime + SEC2MS( accelTime ) {
//				pct = ( gameLocal.time - launchTime ) / SEC2MS( accelTime );
...
//=================
//*/
//void idSoulCubeMissile::Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower, const bool primary ) {
//	idVec3		newStart;
//	idVec3		offs;
//	idEntity	*ownerEnt;
//
...
void idBFGProjectile::Think( void )
{
	if( state == LAUNCHED )
	{
		...
		if( spawnArgs.GetBool( "secondary", "0" ) )
		{
			idVec3		muzzleOrigin;
			idMat3		muzzleAxis;
			idPlayer*	player = static_cast<idPlayer*>( owner.GetEntity() );
			...
			player->weapon.GetEntity()->GetGlobalJointTransform( true, player->weapon.GetEntity()->GetBarrelJointView(), muzzleOrigin, muzzleAxis );
			SetOrigin( muzzleOrigin + ( player->firstPersonViewAxis.ToAngles().ToForward() * 16.0f ) );	// light origin
			SetAngles( player->firstPersonViewAxis.ToAngles() );				// light angle

			if( player->AI_ATTACK_HELD && player->GetEnergy() > player->weapon.GetEntity()->spawnArgs.GetInt( "minEnergy", "20" ) )
			{
				if( gameLocal.time > nextDamageTime )
				{
					player->GiveEnergy( -spawnArgs.GetInt( "energyAmount", "1" ) );
				}
			}
			else
			{
		...
		UpdateBeamTargets();	// sikk - BFG Modifactions
		UpdateVisuals();
	}
...
=================
*/
void idBFGProjectile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float power, const float dmgPower, const bool primary )
{
...
void idBFGProjectile::Explode( const trace_t& collision, idEntity* ignore )
{
	...
	for( i = 0; i < beamTargets.Num(); i++ )
	{
		if( ( beamTargets[i].target == NULL ) || ( ownerEnt == NULL ) )
		{
			continue;
		}

		if( !beamTargets[i].target->CanDamage( GetPhysics()->GetOrigin(), dmgPoint ) )
		{
			continue;
		}
		...
		{
			// if the projectile hit an actor
			if( beamTargets[ i ].target->IsType( idActor::Type ) )
			{
		...
		//if ( damage[0] && ( beamTargets[i].target->entityNumber > gameLocal.numClients - 1 ) ) {
		//	dir = beamTargets[i].target->GetPhysics()->GetOrigin() - GetPhysics()->GetOrigin();
		//	dir.Normalize();
		//	beamTargets[i].target->Damage( this, ownerEnt, dir, damage, damageScale, ( collision.c.id < 0 ) ? CLIPMODEL_ID_TO_JOINT_HANDLE( collision.c.id ) : INVALID_JOINT );
		//}
	}
	...
	// sikk - enable splash damage when projectile hits world
	//if ( ignore == NULL ) {
...
// ---> sikk - BFG Modifactions
/*
================
idBFGProjectile::UpdateBeamTargets
================
*/
void idBFGProjectile::UpdateBeamTargets()
{
	int			numListedEntities;
	float		radius, cosine;
	idPlayer*	player;
	...
	idVec3		muzzleOrigin;
	idMat3		muzzleAxis;
	bool		secondary = spawnArgs.GetBool( "secondary", "0" );

	radius = spawnArgs.GetFloat( "damageRadius", "512" );
	cosine = spawnArgs.GetFloat( "damageCone", "0.5" );
	bounds = idBounds( GetPhysics()->GetOrigin() ).Expand( radius );
	numListedEntities = gameLocal.clip.EntitiesTouchingBounds( bounds, CONTENTS_BODY, entityList, MAX_GENTITIES );

	// this is a player only weapon so owner will always be a player
	player = static_cast<idPlayer*>( owner.GetEntity() );
	idVec3 projOrigin = secondary ? player->firstPersonViewOrigin : GetPhysics()->GetOrigin();

	// remove target entity from list if it's dead, beyond the damage radius, or outside the cone of fire (secondary only)
	for( int i = 0; i < beamTargets.Num(); i++ )
	{
		idVec3 dist = beamTargets[ i ].target->GetPhysics()->GetAbsBounds().GetCenter() - projOrigin;
		float length = dist.Normalize();
		float cos = secondary ? dist * player->firstPersonViewAxis[ 0 ] : 1.0f;

		if( length > radius || cos < cosine || beamTargets[ i ].target->health <= 0 )
		{
			gameRenderWorld->FreeEntityDef( beamTargets[ i ].modelDefHandle );
			beamTargets[ i ].modelDefHandle = -1;
			beamTargets.RemoveIndex( i );
			i--;
		}
	}

	// loop through entities touching the bounds and add them to the list
	for( int i = 0; i < numListedEntities; i++ )
	{
		ent = entityList[ i ];
		assert( ent );

		idVec3 dist = ent->GetPhysics()->GetAbsBounds().GetCenter() - projOrigin;
		float length = dist.Normalize();
		float cos = secondary ? dist * player->firstPersonViewAxis[ 0 ] : 1.0f;
		if( length > radius || cos < cosine )
		{
			continue;
		}

		// if ent is already targeted, skip
		bool skip = false;
		for( int j = 0; j < beamTargets.Num(); j++ )
		{
			if( beamTargets[ j ].target == ent )
			{
				skip = true;
			}
		}
		...
		// if any of these are true, skip
		if( ent == this || ent == owner.GetEntity() || ent->IsHidden() || !ent->IsActive() || !ent->fl.takedamage ||
				ent->health <= 0 || !ent->IsType( idActor::Type ) || !ent->CanDamage( GetPhysics()->GetOrigin(), damagePoint ) )
		{
			continue;
		}

		// if player is targeted, do BFG Vision
		if( ent->IsType( idPlayer::Type ) )
		{
			idPlayer* player = static_cast<idPlayer*>( ent );
			player->playerView.EnableBFGVision( true );
	...
	for( int i = 0; i < beamTargets.Num(); i++ )
	{
		// if entity doesn't exist or is dead, remove it from the list
		if( beamTargets[i].target == NULL || beamTargets[i].target->health <= 0 )
		{
			gameRenderWorld->FreeEntityDef( beamTargets[ i ].modelDefHandle );
			beamTargets[ i ].modelDefHandle = -1;
			beamTargets.RemoveIndex( i );
		...
		idPlayer* entPlayer = ( beamTargets[ i ].target->IsType( idPlayer::Type ) ) ? static_cast<idPlayer*>( beamTargets[i].target ) : NULL;
		idVec3 org = beamTargets[ i ].target->GetPhysics()->GetAbsBounds().GetCenter();
		...
		{
			bool bfgVision = true;
			if( damageFreq && *( const char* )damageFreq && beamTargets[ i ].target && beamTargets[i].target->CanDamage( GetPhysics()->GetOrigin(), org ) )
			{
				org = beamTargets[ i ].target->GetPhysics()->GetOrigin() - GetPhysics()->GetOrigin();
				org.Normalize();
				beamTargets[ i ].target->Damage( this, owner.GetEntity(), org, damageFreq, ( damagePower ) ? damagePower : 1.0f, INVALID_JOINT );

// ---> sikk - Modifier System
				if( owner->IsType( idPlayer::Type ) )
				{
					if( static_cast<idPlayer*>( owner.GetEntity() )->modifiers.weapon_incendiary )
					{
						const idDict* damageDef = gameLocal.FindEntityDefDict( damageFreq );
						if( damageDef )
						{
							beamTargets[ i ].target->burnPower += damageDef->GetInt( "damage" ) * damagePower * 0.1f;
							if( beamTargets[ i ].target->burnPower < 1 )
							{
								beamTargets[ i ].target->burnPower = 1;
							}
						}
						beamTargets[ i ].target->burnTime = gameLocal.time + SEC2MS( g_burnTime.GetFloat() );
					}
				}
// <--- sikk - Modifier System
			...
			if( entPlayer )
			{
				entPlayer->playerView.EnableBFGVision( bfgVision );
			}
//				nextDamageTime = gameLocal.time + BFG_DAMAGE_FREQUENCY;	// sikk - Commented
		}
...
// ---> sikk - Chainsaw Blade Projectile
/*
===============================================================================
...
CLASS_DECLARATION( idProjectile, idChainsawMissile )
END_CLASS

/*
================
idChainsawMissile::Spawn( void )
================
*/
void idChainsawMissile::Spawn( void )
{
	launchTime = 0;
	killPhase = false;
	returnPhase = false;
	...
	beamModelDefHandle = -1;
}

/*
=================
idChainsawMissile::~idChainsawMissile
=================
*/
idChainsawMissile::~idChainsawMissile()
{
	if( beamModelDefHandle != -1 )
	{
		gameRenderWorld->FreeEntityDef( beamModelDefHandle );
		beamModelDefHandle = -1;
	}
}
...
/*
================
idChainsawMissile::Save
================
*/
void idChainsawMissile::Save( idSaveGame* savefile ) const
{
	savefile->WriteInt( launchTime );
	savefile->WriteBool( killPhase );
	savefile->WriteBool( returnPhase );
...
/*
================
idChainsawMissile::Restore
================
*/
void idChainsawMissile::Restore( idRestoreGame* savefile )
{
	savefile->ReadInt( launchTime );
	savefile->ReadBool( killPhase );
	savefile->ReadBool( returnPhase );
...
/*
================
idChainsawMissile::Think
================
*/
void idChainsawMissile::Think( void )
{
	idVec3		seekPos;
	idEntity*	ownerEnt;
...
// ---> sikk - Beam effect
		if( ownerEnt && ownerEnt->IsType( idPlayer::Type ) )
		{
			...
			idMat3 beamAxis = player->GetPhysics()->GetAxis();
			idVec3 beamStart = player->GetPhysics()->GetAbsBounds().GetCenter();
			idVec3 beanEnd = physicsObj.GetOrigin();
			beamRenderEntity.origin = beamStart;
			beamRenderEntity.axis = beamAxis;
			beamRenderEntity.shaderParms[ SHADERPARM_BEAM_END_X ] = beanEnd.x;
			beamRenderEntity.shaderParms[ SHADERPARM_BEAM_END_Y ] = beanEnd.y;
			beamRenderEntity.shaderParms[ SHADERPARM_BEAM_END_Z ] = beanEnd.z;
			beamRenderEntity.shaderParms[ SHADERPARM_BEAM_WIDTH ]	= 2.0f;
			beamRenderEntity.shaderParms[ SHADERPARM_RED ]			= 0.5f;
			beamRenderEntity.shaderParms[ SHADERPARM_GREEN ]		= 0.5f;
			beamRenderEntity.shaderParms[ SHADERPARM_BLUE ]			= 0.5f;
			beamRenderEntity.shaderParms[ SHADERPARM_ALPHA ]		= 1.0f;

			if( beamModelDefHandle != -1 )
			{
				gameRenderWorld->UpdateEntityDef( beamModelDefHandle, &beamRenderEntity );
			}
			else
			{
				memset( &beamRenderEntity, 0, sizeof( renderEntity_t ) );
				beamRenderEntity.hModel = renderModelManager->FindModel( "_beam" );
				beamRenderEntity.callback = NULL;
				beamRenderEntity.numJoints = 0;
				beamRenderEntity.joints = NULL;
				beamRenderEntity.bounds.Clear();
				beamRenderEntity.customSkin = declManager->FindSkin( spawnArgs.GetString( "skin" ) );
				beamModelDefHandle = gameRenderWorld->AddEntityDef( &beamRenderEntity );
			}
		}
// <--- sikk - Beam effect

// ---> sikk -
		if( ( ownerEnt->GetPhysics()->GetOrigin() - physicsObj.GetOrigin() ).Length() > spawnArgs.GetFloat( "clamp_dist" ) )
		{
			ReturnToOwner();
		}
		...
		if( returnPhase )
		{
			idVec3		dir, velocity, nose, tmp;
			idMat3		axis;
			idAngles	dirAng, diff;
			float		dist;

			seekPos = ownerEnt->GetPhysics()->GetAbsBounds().GetCenter();
			nose = physicsObj.GetOrigin();// + 10.0f * physicsObj.GetAxis()[0];
			...
			// clamp the to the max turn rate
			//float turn_max = spawnArgs.GetFloat( "turn_max", "180" );
			//for ( int i = 0; i < 3; i++ ) {
			//	if ( diff[ i ] > turn_max ) {
			//		diff[ i ] = turn_max;
			//	} else if ( diff[ i ] < -turn_max ) {
			...
			// make the visual model always points the dir we're traveling
			//dir = angles.ToForward();
			velocity = dir * speed * 2.0f;	// we return twice as fast
			physicsObj.SetLinearVelocity( velocity );
			physicsObj.SetAngularVelocity( idVec3( 0.0f, 0.0f, 0.0f ) );
...
/*
================
idChainsawMissile::ReturnToOwner
================
*/
void idChainsawMissile::ReturnToOwner()
{
	physicsObj.SetClipMask( 0 );
	killPhase = false;
	returnPhase = true;
...
/*
=================
idChainsawMissile::Launch
=================
*/
void idChainsawMissile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower, const bool primary )
{
	idVec3		newStart;
	idVec3		offs;

	newStart = owner.GetEntity()->GetPhysics()->GetAbsBounds().GetCenter();
	// push it out a little
	newStart = newStart + dir * spawnArgs.GetFloat( "launchDist" );
	offs = spawnArgs.GetVector( "launchOffset", "0 0 -4" );
	newStart += offs;
	idProjectile::Launch( newStart, dir, pushVelocity, timeSinceFire, launchPower, dmgPower );
...
//	physicsObj.SetClipMask( 0 ); // never collide.. think routine will decide when to detonate
	physicsObj.SetLinearVelocity( speed * physicsObj.GetAxis()[2] );
	launchTime = gameLocal.time;
	killPhase = false;
	UpdateVisuals();
}
...
/*
=================
idChainsawMissile::Collide
=================
*/
bool idChainsawMissile::Collide( const trace_t& collision, const idVec3& velocity )
{
	idEntity*	ent;
	idEntity*	ignore;
	const char*	damageDefName;
...
CLASS_DECLARATION( idProjectile, idVagaryMissile )
END_CLASS

/*
================
idVagaryMissile::Spawn( void )
================
*/
void idVagaryMissile::Spawn( void )
{
}

/*
=================
idVagaryMissile::~idVagaryMissile
=================
*/
idVagaryMissile::~idVagaryMissile()
{
...
/*
================
idVagaryMissile::Save
================
*/
void idVagaryMissile::Save( idSaveGame* savefile ) const
{
}

/*
================
idVagaryMissile::Restore
================
*/
void idVagaryMissile::Restore( idRestoreGame* savefile )
{
}

/*
================
idVagaryMissile::Think
================
*/
void idVagaryMissile::Think( void )
{
	idProjectile::Think();
}
...
/*
=================
idVagaryMissile::Collide
=================
*/
bool idVagaryMissile::Collide( const trace_t& collision, const idVec3& velocity )
{
	idEntity*	ent;
	idEntity*	ignore;
	const char*	damageDefName;
...
// <--- sikk - Modifier System
	...
	// spawn web trigger entity
	idEntity* webEnt;
	idDict args;
	args.Set( "classname", "trigger_sticky" );
	args.Set( "size", "128 128 128" );
	args.Set( "stickyness", "1.0" );
	args.SetVector( "origin", collision.endpos );
	gameLocal.SpawnEntityDef( args, &webEnt );
	if( webEnt )
	{
		webEnt->SetOrigin( collision.endpos );
		//	webEnt->SetAxis( collision.endAxis );
		//	webEnt->SetAngles( collision.c.normal.ToAngles() );
		webEnt->PostEventMS( &EV_Remove, 5000 );
	}
