	...
	switch( gameState )
	{
		case GAMEREVIEW:
		{
			if( nextState == INACTIVE )
			{
				...
				nextState = NEXTGAME;
				nextStateSwitch = gameLocal.time + SEC2MS( gameReviewPause );
			}
			break;
		}
		case NEXTGAME:
		...
		case WARMUP:
		{
			if( AllPlayersReady() )
			{
				NewState( COUNTDOWN );
				nextState = GAMEON;
				nextStateSwitch = gameLocal.time + SEC2MS( cvarSystem->GetCVarInteger( "g_countDown" ) );
			}
...
void idMultiplayerGame::ServerCallVote( int clientNum, const idBitMsg& msg )
{
	...
	switch( voteIndex )
	{
		...
		case VOTE_MAP:
		{
			...
			int				i;
			const idDict*	dict = NULL;	// sikk - warning C4701: potentially uninitialized local variable used
			bool			haveMap = false;
			for( i = 0; i < num; i++ )
			{
				dict = fileSystem->GetMapDecl( i );
...
void idMultiplayerGame::ThrottleUserInfo( void )
{
	...
	while( ThrottleVars[ i ] )
	{
						 ...
						 cvarSystem->GetCVarString( ThrottleVars[ i ] ) ) )
		{
			...
			{
				switchThrottle[ i ] = gameLocal.time + SEC2MS( ThrottleDelay[ i ] );
			}
		}
		i++;
	}
