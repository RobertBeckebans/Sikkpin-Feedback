...
void idLiquid::Save( idSaveGame* savefile ) const
{
	...
	savefile->WriteParticle( waves );
	for( int i = 0; i < 3; i++ )
	{
		savefile->WriteParticle( splash[ i ] );
	}
}
...
//	Spawn();
	...
	savefile->ReadString( smokeName );
	savefile->ReadString( soundName );
	savefile->ReadParticle( waves );
	for( int i = 0; i < 3; i++ )
	{
		savefile->ReadParticle( splash[ i ] );
	}
}
...
void idLiquid::Spawn()
{
	...
	*/
	...
	// setters
	smokeName = "smoke_";
	smokeName.Append( temp );

	soundName = "snd_";
	soundName.Append( temp );

	splash[ 0 ] = static_cast<const idDeclParticle*>( declManager->FindType( DECL_PARTICLE, spawnArgs.GetString( "smoke_small", "water_splash_tiny" ) ) );
	splash[ 1 ] = static_cast<const idDeclParticle*>( declManager->FindType( DECL_PARTICLE, spawnArgs.GetString( "smoke_medium", "water_splash" ) ) );
	splash[ 2 ] = static_cast<const idDeclParticle*>( declManager->FindType( DECL_PARTICLE, spawnArgs.GetString( "smoke_large", "water_splash_large" ) ) );

	waves = static_cast<const idDeclParticle*>( declManager->FindType( DECL_PARTICLE, spawnArgs.GetString( "smoke_waves", "water_waves" ) ) );

	// setup physics
	physicsObj.SetSelf( this );
	physicsObj.SetClipModel( new idClipModel( GetPhysics()->GetClipModel() ), liquidDensity );
	physicsObj.SetOrigin( GetPhysics()->GetOrigin() );
	physicsObj.SetAxis( GetPhysics()->GetAxis() );
	physicsObj.SetGravity( gameLocal.GetGravity() );
	physicsObj.SetContents( CONTENTS_WATER | CONTENTS_TRIGGER );

	physicsObj.SetDensity( liquidDensity );
	physicsObj.SetViscosity( liquidViscosity );
	physicsObj.SetMinSplashVelocity( minSplash );
	physicsObj.SetMinWaveVelocity( minWave );
...
bool idLiquid::Collide( const trace_t& collision, const idVec3& velocity )
{
	...
	float eMass = ent->GetPhysics()->GetMass();
	idPhysics_Liquid* phys = static_cast<idPhysics_Liquid*>( GetPhysics() );
	const idDeclParticle* pSplash;
	const char* sName;

	if( velocity.LengthSqr() > phys->GetMinSplashVelocity().LengthSqr() )
	{
		// pick which splash particle to spawn
		// first we check the entity, if it's not defined we use
		// one defined for this liquid.
		sName = ent->spawnArgs.GetString( smokeName.c_str() );
		if( *sName != '\0' )
		{
			// load entity particle
			pSplash = static_cast<const idDeclParticle*>( declManager->FindType( DECL_PARTICLE, sName ) );
		}
		else
		{
		...
		ent->StartSound( soundName.c_str(), SND_CHANNEL_ANY, 0, false, NULL );
	}
	else if( velocity.LengthSqr() > phys->GetMinWaveVelocity().LengthSqr() )
	{
		pSplash = waves;
	}
	else
	...
	// spawn the particle
	gameLocal.smokeParticles->EmitSmoke( pSplash, gameLocal.time, gameLocal.random.RandomFloat(), collision.c.point, collision.c.normal.ToMat3() );
