...
public:
	...
	{
		return ( gameLocal.clip.Contents( point, NULL, mat3_identity, MASK_WATER, NULL ) != 0 );
	}
	virtual idVec3		GetDepth( const idVec3& point ) const;
	virtual idVec3		GetPressure( const idVec3& point ) const
	{
		return GetDepth( point ) * density;
	}
	...
	virtual void		SetDensity( float _density )
	{
		if( _density > 0.0f )
		{
			density = _density;
	...
	virtual void		SetViscosity( float _viscosity )
	{
		if( _viscosity >= 0.0f )
		{
	...
	{
		return minSplashVelocity;
	}
	virtual void		SetMinSplashVelocity( const idVec3& velocity )
	{
		minSplashVelocity = velocity;
	}

	virtual const idVec3& GetMinWaveVelocity() const
	{
		return minWaveVelocity;
	}
	virtual void		SetMinWaveVelocity( const idVec3& velocity )
	{
		minWaveVelocity = velocity;
	}
