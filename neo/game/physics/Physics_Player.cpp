...
const float PM_ACCELERATE		= 20.0f;	// sikk - was 10.0f;
const float PM_AIRACCELERATE	= 5.0f;		// sikk - was 1.0f;
const float PM_WATERACCELERATE	= 4.0f;
const float PM_FLYACCELERATE	= 8.0f;

const float PM_FRICTION			= 10.0f;
const float PM_CROUCHFRICTION	= 1.0f;		// sikk - Crouch Friction
const float PM_AIRFRICTION		= 0.1f;		// sikk - was 0.0f;
...
// ---> sikk - Crouch Slide
				float lerp = idMath::ClampFloat( 0.0f, 1.0f, current.velocity.Length() * 0.003125f );
				float couchfriction = PM_CROUCHFRICTION * lerp + PM_FRICTION * ( 1.0f - lerp );
				drop += control * ( ( current.movementFlags & PMF_DUCKED ) ? couchfriction : PM_FRICTION ) * frametime;
// <--- sikk - Crouch Slide
			}
		}
	}
...
// ---> sikk - Wall Jump
	if( idPhysics_Player::CheckWallJump() )
	{
		// jumped away
		idPhysics_Player::AirMove();
		return;
...
// ---> sikk - Modifier System: Double Jump
	if( canDoubleJump && doubleJumpActive && ( current.velocity * gravityNormal ) < 0.0f )
	{
...
void idPhysics_Player::WalkMove()
{
	...
	{
		accelerate = pm_airAccelerate;	// sikk - Modifier System;
	}
	else
	{
...
void idPhysics_Player::CheckGround()
{
	...
	doubleJumpActive = true;	// sikk - Modifier System: Double Jump
...
// ---> sikk - Ladder requries Jump to be held
...
// <--- sikk - Ladder requries Jump to be held
	...
	if( trace.fraction < 1.0f )
	{
		...
		if( trace.c.material && ( trace.c.material->GetSurfaceFlags() & SURF_LADDER ) )
		{
			...
			if( trace.fraction < 1.0f )
			{
				// sikk - make sure view is within 45 degrees of surface normal
				float cos = forward * trace.c.normal;

				// if it also is a ladder surface
				if( trace.c.material && trace.c.material->GetSurfaceFlags() & SURF_LADDER && cos < -0.70710678 )
				{
					ladder = true;
...
=============
*/
bool idPhysics_Player::CheckWallJump()
{
	idVec3 addVelocity;
	...
	trace_t	trace;
	float	traceDist, cos;
	idVec3	forward, start, end, traceDir;
	idBounds bounds;
	...
	end = start + traceDist * traceDir;
	bounds = idBounds( idVec3( -16.0f, -16.0f, 0.0f ), idVec3( 16.0f, 16.0f, 32.0f ) );
	//gameLocal.clip.TracePoint( trace, start, end, clipModel, clipMask, self );
	...
	if( trace.fraction < 1.0f )
	{
		cos = forward * trace.c.normal;
		if( idMath::Fabs( cos ) > 0.5f || current.velocity.z > -32.0f )
		{
			return false;
		}
		...
		addVelocity = 2000.0f * maxJumpHeight * trace.c.normal;
		addVelocity *= idMath::Sqrt( addVelocity.Normalize() );
		current.velocity += addVelocity;
		return true;
	}
...
// ---> sikk - Liquid Physics: defined in "Physics_Actor"
/*
=============
...
// ---> sikk - Modifier System
	savefile->WriteFloat( pm_airAccelerate );
	savefile->WriteBool( canDoubleJump );
	savefile->WriteBool( doubleJumpActive );
// <--- sikk - Modifier system
...
// ---> sikk - Modifier System
	savefile->ReadFloat( pm_airAccelerate );
	savefile->ReadBool( doubleJumpActive );
	savefile->ReadBool( canDoubleJump );
// <--- sikk - Modifier system
...
/*
================
idPhysics_Player::SetAirAcceleration
================
*/
void idPhysics_Player::SetAirAcceleration( const float accel )
{
	pm_airAccelerate = accel;
}

/*
================
idPhysics_Player::SetDoubleJump
================
*/
void idPhysics_Player::SetDoubleJump( const bool doubleJump )
{
