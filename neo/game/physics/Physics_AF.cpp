...
const float NO_MOVE_TIME					= 2.0f;	// sikk - Liquid Physics - was 1.0f
...
idAFBody::SetWaterLevel
	returns the percent of the body in water 0.0f if out of water

	Note: we use the liquid's gravity normal for floating because
	the idPhysics_AF gravity normal is really hard to get a hold of!
================
*/
float idAFBody::SetWaterLevel( idPhysics_Liquid* l, const idVec3& gravityNormal, bool fixedDensityBuoyancy )
{
	...
	{
		idVec3 depth, bottom( current->worldOrigin );
		idBounds bounds = clipModel->GetBounds();
		float height, d;

		// offset and rotate the bounding box
		bounds += -centerOfMass;
		bounds *= current->worldAxis.Transpose();
		...
		if( d > height )	// the body is totally submerged
		{
			waterLevel = 1.0f;
		}
		...
		else	// the body is partly submerged
		{
			waterLevel = d / height;
		}
	}
...
// ---> sikk - Liquid Physics
		...
		if( water != NULL && body->GetWaterLevel() > 0.0f )
		{
...
// ---> sikk - Liquid Physics
		...
		float waterLevel = body->GetWaterLevel();
		if( waterLevel == 0.0f || water == NULL )
		{
			body->next->spatialVelocity.SubVec3( 0 ) -= body->linearFriction * body->next->spatialVelocity.SubVec3( 0 );
		}
		else
		{
			body->next->spatialVelocity.SubVec3( 0 ) -= body->linearFriction * body->next->spatialVelocity.SubVec3( 0 ) * waterLevel * ( water->GetViscosity() + WATER_FRICTION );
		}
// <--- sikk - Liquid Physics
...
// ---> sikk - Liquid Physics
	float invMass = ( water != NULL ) ? body->invLiquidMass : body->invMass;
...
// ---> sikk - Liquid Physics
									   ...
									   body->clipModel, body->current->worldAxis, MASK_WATER, passEntity ) )
			{
				...
				if( ent->GetPhysics()->IsType( idPhysics_Liquid::Type ) )
				{
					...
					self->GetImpactInfo( ent, collision.c.id, collision.c.point, &info );

					SetWater( liquid );
					water->Splash( self, body->GetVolume(), info, collision );
				}
			}
// <--- sikk - Liquid Physics
...
// ---> sikk - Liquid Physics
	idVec3 grav( liquidDensity * gravityVector );
	float waterLevel, wDensity = 0.0f;
	bool inWater, bodyBuoyancy = false;

	if( SetWaterLevelf() == 1.0f )
	{
		wDensity = water->GetDensity();
...
// ---> sikk - Liquid Physics
		waterLevel = body->SetWaterLevel( water, gravityNormal, fixedDensityBuoyancy );
		if( waterLevel > 0.0f )
		{
			if( !fixedDensityBuoyancy && !bodyBuoyancy )
			{
			...
			{
				body->liquidMass = body->volume * liquidDensity * LIQUID_MASS_MUL;
...
// ---> sikk - Liquid Physics
	// if all AFBodies are not in the water, we assume the whole entity is not in water so we clear the water flag
	if( !inWater )
	{
		water = NULL;
	}
...
// ---> sikk - Liquid Physics
	...
	{
		fixedDensityBuoyancy = true;
		liquidDensity = DEFAULT_LIQUID_DENSITY;
	}
	else
	{
		fixedDensityBuoyancy = false;
		liquidDensity = DEFAULT_LIQUID_SCALAR;
	}
	water = NULL;
// <--- sikk - Liquid Physics
}
...
// ---> sikk - Liquid Physics
	saveFile->WriteBool( fixedDensityBuoyancy );
	saveFile->WriteFloat( liquidDensity );
// <--- sikk - Liquid Physics
}
...
// ---> sikk - Liquid Physics
	saveFile->ReadBool( fixedDensityBuoyancy );
	saveFile->ReadFloat( liquidDensity );
// <--- sikk - Liquid Physics
