...
// sikk - Level Stats System: EndLevel used to show scores and won't precede to next level until MOUSE1 is pressed
	if( thinkFlags & TH_THINK )
	{
		player = gameLocal.GetLocalPlayer();
		player->stats->Redraw( gameLocal.realClientTime );
...
//			player->SelectWeapon( player->weapon_fists, true );	// sikk
		}
	}
}
...
void idTarget_FadeSoundClass::Event_RestoreVolume()
{
	float fadeTime = spawnArgs.GetFloat( "fadeTime" );
	float fadeDB = spawnArgs.GetFloat( "fadeDB" );
//	int fadeClass = spawnArgs.GetInt( "fadeClass" );	// sikk - warning C4189: 'fadeClass' : variable initialized but not used (should be used as an arg below?)
	// restore volume
