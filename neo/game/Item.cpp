...
// ---> sikk - Recharger Pack System
//	player->GivePowerUp( type, SEC2MS( time ) );
//	return true;
	return player->GivePowerUp( type, SEC2MS( time ), spawnArgs.GetInt( "score" ) );	// sikk - Level Stats System: Added score
// <--- sikk - Recharger Pack System
}
...
void idMoveableItem::Think()
{
	...
	if( ( thinkFlags & TH_UPDATEPARTICLES ) && GetPhysics()->GetLinearVelocity().Length() > 100.0f )
	{
		...
		{
			smokeTime = gameLocal.time;	// sikk - continuous smoke particles (for gibs)
