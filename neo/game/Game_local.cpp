...
// ---> sikk - Level Stats System: Enemy Count/Secrets Count
	enemyCount			 = 0;	// sikk - Level Stats System: Enemy Count
	secretsCount		 = 0;	// sikk - Level Stats System: Secrets Count
...
// ---> sikk - Level Stats System: Enemy Count/Secrets Count
	savegame.WriteInt( enemyCount );
	savegame.WriteInt( secretsCount );
// <--- sikk - Level Stats System
...
// ---> sikk - Level Stats System: Enemy Count/Secrets Count
	savegame.ReadInt( enemyCount );
	savegame.ReadInt( secretsCount );
// <--- sikk - Level Stats System
...
// ---> sikk - automatic aspect ratio
		default : // automatic
			ratio_x = 16.0f;
			ratio_y = 16.0f * renderSystem->GetScreenHeight() / renderSystem->GetScreenWidth();
			break;
// <--- sikk - automatic aspect ratio
	}
...
// ---> sikk - Level Stats System: Enemy Count/Secrets Count
			if( !idStr::Icmpn( args.GetString( "classname" ), "monster_", 8 ) )
			{
				enemyCount++;
			}
			else if( !idStr::Icmp( args.GetString( "classname" ), "func_spawner" ) )
			{
				enemyCount += args.GetInt( "count" );
			}
			else if( !idStr::Icmp( args.GetString( "classname" ), "trigger_secret" ) )
			{
				secretsCount++;
// <--- sikk - Level Stats System
// ---> sikk - target_endlevel position
			}
			else if( !idStr::Icmp( args.GetString( "classname" ), "target_endlevel" ) )
			{
				for( int i = 0; i < 4; i++ )
				{
...
// ---> sikk - Explosion FX PostProcess
	...
	explosionTime = time + SEC2MS( g_explosionFXTime.GetInteger() );
// <--- sikk - Explosion FX PostProcess
...
//	bool		tracer;
	...
	if( owner && owner->IsType( idPlayer::Type ) )
	{
		damageScale *= static_cast<idPlayer*>( owner )->PowerUpActive( MEGADAMAGE ) * 3.0f + 1.0f;
	}
...
// ---> sikk - Rail Charge
			if( hitscanDict.GetInt( "railCharge" ) )
			{
				if( owner->IsType( idPlayer::Type ) && ent->IsType( idAFEntity_Gibbable::Type ) && ent != owner && owner->health > 0 )
				{
					static_cast<idAFEntity_Gibbable*>( ent )->railCharge += hitscanDict.GetInt( "railCharge" );
					static_cast<idAFEntity_Gibbable*>( ent )->railChargeTime = gameLocal.time + 1000;
					if( static_cast<idAFEntity_Gibbable*>( ent )->railCharge > 100 )
					{
						static_cast<idAFEntity_Gibbable*>( ent )->railCharge = 100;
					}
				}
			}
// <--- sikk - Rail Charge
			...
			if( damage && damage[ 0 ] )
			{
				// RAVEN BEGIN
				// ddynerman: stats
				if( owner->IsType( idPlayer::Type ) && ent->IsType( idActor::Type ) && ent != owner && owner->health > 0 )
				{
					idPlayer* player = static_cast<idPlayer*>( owner );

// ---> sikk - Modifier System
					if( player->modifiers.weapon_incendiary )
					{
						const idDict* damageDef = gameLocal.FindEntityDefDict( damage );
						if( damageDef )
						{
...
// ---> sikk - Blood Spray Screen Effect
					...
					{
						idPlayer* player = ( idPlayer* )owner;
