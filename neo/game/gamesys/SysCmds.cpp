...
// ---> sikk - Energy System
		if( g_energySystem.GetBool() )
		{
			...
			player->energy = player->inventory.maxEnergy;
		}
		else
		{
			player->health = player->inventory.maxHealth + player->inventory.maxEnergy;
		}
// <--- sikk - Energy System
		if( !give_all )
		{
	...
	if( idStr::Icmp( name, "megadamage" ) == 0 )
	{
		player->GivePowerUp( MEGADAMAGE, SEC2MS( 30.0f ) );
		return;
	}

	if( idStr::Icmp( name, "megashield" ) == 0 )
	{
		player->GivePowerUp( MEGASHIELD, SEC2MS( 30.0f ) );
		return;
	}

	if( idStr::Icmp( name, "megaenergy" ) == 0 )
	{
		player->GivePowerUp( MEGADAMAGE, SEC2MS( 30.0f ) );
		return;
	}

	if( idStr::Icmp( name, "megacharge" ) == 0 )
	{
		player->GivePowerUp( MEGASHIELD, SEC2MS( 0.0f ) );
		return;
	}
