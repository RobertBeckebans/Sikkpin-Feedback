...
// <--- sikk - raynorpat's GLSL Support
	...
	//const struct viewDef_s *prevviewDef = backEnd.previousviewDef;
	//float prevMat[ 16 ];
	//for ( int i = 0; i < 16; i++ ) {
	//	prevMat[ i ] = backEnd.previousSpace->modelViewMatrix[ i ];
	//	backEnd.previousSpace->modelViewMatrix[ i ] = space->modelViewMatrix[ i ];	// sikk - save previous space info for motion blur
	//}
	//backEnd.previousviewDef = backEnd.viewDef;	// sikk - save previous space info for motion blur
	float parm[ 4 ];
...
// ---> sikk - Postion to View space - for depth buffer related stuff
	...
	// positionToViewSpace
	parm[0] = -1.0f / backEnd.viewDef->projectionMatrix[0];
	parm[1] = -1.0f / backEnd.viewDef->projectionMatrix[5];
	parm[2] = 2.0f / ( backEnd.viewDef->viewport.x2 - backEnd.viewDef->viewport.x1 + 1 );
	parm[3] = 2.0f / ( backEnd.viewDef->viewport.y2 - backEnd.viewDef->viewport.y1 + 1 );
	qglProgramEnvParameter4fvARB( GL_FRAGMENT_PROGRAM_ARB, 21, parm );
// <--- sikk - Postion to View space - for depth buffer related stuff


// ---> sikk - previous frames modelView matrix
//	if ( prevMat != NULL ) {
	//parm[0] = prevMat[0];
	//parm[1] = prevMat[4];
	//parm[2] = prevMat[8];
	//parm[3] = prevMat[12];
	//qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 9, parm );
	//parm[0] = prevMat[1];
	//parm[1] = prevMat[5];
	//parm[2] = prevMat[9];
	//parm[3] = prevMat[13];
	//qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 10, parm );
	//parm[0] = prevMat[2];
	//parm[1] = prevMat[6];
	//parm[2] = prevMat[10];
	//parm[3] = prevMat[14];
	//qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 11, parm );
	//parm[0] = prevMat[3];
	//parm[1] = prevMat[7];
	//parm[2] = prevMat[11];
	//parm[3] = prevMat[15];
	//qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 12, parm );

	/*		parm[0] = prevviewDef->projectionMatrix[0];
			parm[1] = prevviewDef->projectionMatrix[4];
			parm[2] = prevviewDef->projectionMatrix[8];
			parm[3] = prevviewDef->projectionMatrix[12];
			qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 13, parm );
			parm[0] = prevviewDef->projectionMatrix[1];
			parm[1] = prevviewDef->projectionMatrix[5];
			parm[2] = prevviewDef->projectionMatrix[9];
			parm[3] = prevviewDef->projectionMatrix[13];
			qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 14, parm );
			parm[0] = prevviewDef->projectionMatrix[2];
			parm[1] = prevviewDef->projectionMatrix[6];
			parm[2] = prevviewDef->projectionMatrix[10];
			parm[3] = prevviewDef->projectionMatrix[14];
			qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 15, parm );
			parm[0] = prevviewDef->projectionMatrix[3];
			parm[1] = prevviewDef->projectionMatrix[7];
			parm[2] = prevviewDef->projectionMatrix[11];
			parm[3] = prevviewDef->projectionMatrix[15];
			qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 16, parm );
	*/
//	}
// <--- sikk - previous frames modelView matrix
}
...
// ---> sikk - LEITH: the patented "Carmack's Reverse" code
...
static void RB_T_BlendLight( const drawSurf_t* surf )
{
	...
	if( backEnd.currentSpace != surf->space )
	{
		...
		qglTexGenfv( GL_Q, GL_OBJECT_PLANE, lightProject[3].ToFloatPtr() );
...
// ---> sikk - ssao buffer
	...
	//renderSystem->DrawStretchPic( 0.0f, 0.0f, SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f, 0.0f, 1.0f, 1.0f, declManager->FindMaterial( "render/depth" ) );
	...
	//renderSystem->SetColor4( 16.0f, 0.0f, 1.5f, 0.0f );
	//renderSystem->DrawStretchPic( 0.0f, 0.0f, SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f, 0.0f, 1.0f, 1.0f, declManager->FindMaterial( "postProcess/ssao" ) );
	//globalImages->ssaoImage->CopyFramebuffer( backEnd.viewDef->viewport.x1, backEnd.viewDef->viewport.y1,
	//	backEnd.viewDef->viewport.x2 - backEnd.viewDef->viewport.x1 + 1,
	//	backEnd.viewDef->viewport.y2 - backEnd.viewDef->viewport.y1 + 1, true );
