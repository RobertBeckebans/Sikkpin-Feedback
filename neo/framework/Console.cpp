...
void idConsoleLocal::Clear()
{
	...
	{
		text[i] = ( idStr::ColorIndex( C_COLOR_GRAY ) << 8 ) | ' ';
	}
...
void idConsoleLocal::Linefeed()
{
	...
	for( i = 0; i < LINE_WIDTH; i++ )
	{
		text[( current % TOTAL_LINES )*LINE_WIDTH + i] = ( idStr::ColorIndex( C_COLOR_GRAY ) << 8 ) | ' ';
	}
}
...
void idConsoleLocal::Print( const char* txt )
{
	...
	color = idStr::ColorIndex( C_COLOR_GRAY );
...
void idConsoleLocal::DrawInput()
{
	...
	renderSystem->SetColor( idStr::ColorForIndex( C_COLOR_GRAY ) );
...
void idConsoleLocal::DrawNotify()
{
	...
	renderSystem->SetColor( colorLtGrey );
}
