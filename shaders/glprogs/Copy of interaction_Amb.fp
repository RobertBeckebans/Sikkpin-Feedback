...
# H1 = 1 / sqrt( 2 * pi )
# H2 = -sqrt( 3 / ( 2 * pi ) ) * y
# H3 = sqrt( 3 / ( 2 * pi ) ) * ( z * 2 - 1 )
# H4 = -sqrt( 3 / ( 2 * pi ) ) * x
# H5 = sqrt( 15 / ( 2 * pi ) ) * x * y
# H6 = 0.5 * sqrt( 15 / ( 2 * pi ) ) * ( x * x - y * y )
PARAM H1 =  0.39894228040143267793994605993438;
PARAM H2 = -0.69098829894267095853048929206378;
PARAM H3 =  0.69098829894267095853048929206378;
PARAM H4 = -0.69098829894267095853048929206378;
PARAM H5 =  1.545096808092758321368770941246;
PARAM H6 =  0.77254840404637916068438547062298;

TEMP	C4, C5, C6;

MUL 	C1, H1, ambColZPos;

MUL 	C2, H2, wNormalVec.y;
MUL 	C2, C2, ambColYPos;

MAD 	R1, wNormalVec.z, 2.0, -1.0;
MUL 	C3, H3, R1;
MUL 	C3, C3, ambColZNeg;

MUL 	C4, H4, wNormalVec.x;
MUL 	C4, C4, ambColXPos;

MUL 	R1, wNormalVec.x, wNormalVec.y;
MUL 	C5, H5, R1;
MUL 	C5, C5, ambColXNeg;

MUL 	R1, wNormalVec, wNormalVec;
ADD 	R1, R1.x, -R1.y;
MUL 	C6, H6, R1;
MUL 	C6, C6, ambColYNeg;

ADD 	ambient, C1, C2;
ADD 	ambient, ambient, C3;
ADD 	ambient, ambient, C4;
ADD 	ambient, ambient, C5;
ADD 	ambient, ambient, C6;
